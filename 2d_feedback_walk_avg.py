...
for t in range(tmax):
    ...
    for x in range(dist.shape[0]):
        for y in range(dist.shape[1]):
            ...
            dist[int(min(x+1,dist.shape[0]-1)),y] += right
            dist[int(max(x-1,0)),y] += left
            dist[x,int(min(y+1,dist.shape[1]-1))] += up
            dist[x,int(max(y-1,0))] += down


    pright = prev / np.max(prev)
    pright = conv2(pright,np.ones((3,3)),'same')/9.

    #pdown = prev / np.max(prev)
