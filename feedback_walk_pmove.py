...
for t in range(tmax):
    ...
    for pos in range(dist.shape[0]):
        ...
        moving = np.sum(np.where(roll < pmove[pos], np.ones_like(roll), np.zeros_like(roll)))
        dist[pos] += roll.shape[0] - moving

        roll = np.random.rand(int(moving))
